...
import type { AWBData, Currency, JobType, NTR } from '../../types'
...
export default function CreateOrderModal({ open, onClose }: Props) {
  ...
  const mutation = useMutation({
    ...
    onError: () => setError('Ошибка при создании заказа'),
  })
    ...
    try {
      ...
      if (Object.keys(extracted).length > 0) {
        ...
        if (extracted.number_of_pieces)
          setCargo(p => ({ ...p, pieces: String(extracted.number_of_pieces) }))
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    if (!main.client_id) { setError('Выберите клиента (CUSTOMER)'); return }
    setError('')
    mutation.mutate({
      // Tab 1
  ...
  return (
    <Modal open={open} onClose={onClose} title="Новый заказ" size="xl">
      {/* Tabs */}
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className={tab === 'main' ? 'space-y-4' : 'hidden'}>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className={lbl}>ASSIGNED (ответственный)</label>
              <select value={main.assigned_to_id}
                onChange={e => setMain(p => ({...p, assigned_to_id: e.target.value}))}
                className={inp}>
        ...
        <div className={tab === 'cargo' ? 'space-y-4' : 'hidden'}>
          ...
          <div className="border border-gray-200 rounded-lg p-4 bg-gray-50">
            ...
            <div className="grid grid-cols-3 gap-3">
              <div>
                <label className={lbl}>NTR (природа груза)</label>
              ...
              <div>
                <label className={lbl}>#PC (количество мест)</label>
                <input type="number" min="1" value={cargo.pieces}
                  onChange={e => setCargo(p => ({...p, pieces: e.target.value}))}
                  className={inp} />
              </div>
              <div>
                <label className={lbl}>KG (фактический вес)</label>
                <input type="number" min="0" step="0.01" value={cargo.weight_kg}
                  onChange={e => setCargo(p => ({...p, weight_kg: e.target.value}))}
                  className={inp} placeholder="0.00" />
              </div>
              <div>
                <label className={lbl}>CWT (оплачиваемый вес)</label>
              ...
              <div className="col-span-2">
                <label className={lbl}>DIMS (размеры: ДxШxВ)</label>
                <input value={cargo.dimensions}
                  onChange={e => setCargo(p => ({...p, dimensions: e.target.value}))}
                  className={inp} placeholder="45x32x61 или 77x32x32 44x44x44" />
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className={lbl}>Получатель (ФИО)</label>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className={lbl}>SHIPPER 2 (полные данные)</label>
            ...
            <div>
              <label className={lbl}>CONSIGNEE 2 (полные данные)</label>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className={lbl}>INSTR (инструкции)</label>
              <textarea value={cargo.instr}
                onChange={e => setCargo(p => ({...p, instr: e.target.value}))}
                className={inp} rows={2} placeholder="Специальные инструкции..." />
            </div>
            <div>
              <label className={lbl}>Note (примечание)</label>
              <textarea value={cargo.notes}
                onChange={e => setCargo(p => ({...p, notes: e.target.value}))}
                className={inp} rows={2} placeholder="Дополнительно..." />
            </div>
          </div>
        </div>
        ...
        <div className={tab === 'awb' ? 'space-y-4' : 'hidden'}>
          ...
          {/* Загрузка AWB документа */}
          <div>
            <p className={sec}>Загрузить AWB документ (OCR)</p>
            ...
            {ocrState === 'ocr' && (
              <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg space-y-2">
                <div className="flex items-center gap-2">
                  <Loader2 size={16} className="text-blue-500 animate-spin" />
                  <p className="text-sm text-blue-700">OCR анализ... {ocrProgress}%</p>
                </div>
            ...
            {ocrState === 'done' && (
              <div className="flex items-center gap-3 p-3 bg-green-50 border border-green-200 rounded-lg">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-green-800 truncate">{awbFileName}</p>
                  <p className="text-xs text-green-600">
                    {ocrConfidence >= 90 ? 'Текстовый слой — данные точные'
                      : ocrConfidence > 0 ? `OCR уверенность: ${ocrConfidence}%`
                      : 'Файл сохранён — заполните поля вручную'}
                  </p>
                </div>
                {awbFileURL && (
            ...
            {ocrState === 'idle' && (
              <button type="button" onClick={() => fileInputRef.current?.click()}
                ...
                <div className="text-left">
                  ...
                  <p className="text-xs text-gray-400">PDF с текстом или JPG/PNG — данные извлекаются автоматически</p>
                </div>
              </button>
            )}
            ...
            {awbPreviewURL && ocrState !== 'idle' && (
              <div className="mt-3 border border-gray-200 rounded-lg overflow-hidden">
                <p className="text-xs text-gray-500 px-3 py-1.5 bg-gray-50 border-b">Документ AWB</p>
                {awbIsPDF
                  ? <embed src={awbPreviewURL} type="application/pdf"
                      className="w-full" style={{ height: '300px' }} />
                  : <img src={awbPreviewURL} alt="AWB" className="w-full object-contain max-h-72" />
                }
              </div>
            )}
          </div>
        ...
        <div className={tab === 'finance' ? 'space-y-4' : 'hidden'}>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className={lbl}>AMOUNT (базовая сумма)</label>
            ...
            <div>
              <label className={lbl}>ADD AMOUNT (доп. сборы)</label>
              ...
              <div>
                ...
                <select value={fin.invoice_status}
                  ...
                  {INVOICE_STATUSES.map(s => (
                    <option key={s} value={s}>{s || '— не указан —'}</option>
                  ))}
                </select>
              </div>
